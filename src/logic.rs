use get_arch_url::{download::download_file, toml_parser::doc::Doc};
...
const LXC_URL: &'static str = "https://images.linuxcontainers.org";
...
pub(crate) fn parse_lxc_toml() -> String {
    ...
    let table = r["products"]["archlinux"]["current"][&*ARCH]["default"]
        .as_table()
        .unwrap();
    let products = table["time"].as_array().unwrap();
    let value = products.get(0).unwrap().to_string();
    let (_, value) = cut_quote(&value).unwrap();

    let lxc_path = table[value].as_table().unwrap();
    ...
    let url = [LXC_URL, bb, "rootfs.tar.xz"].concat();
    url
}
